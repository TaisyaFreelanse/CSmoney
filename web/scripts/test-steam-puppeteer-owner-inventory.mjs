/**
 * Локальная проверка: trade-offer под сессией магазина — грузим инвентарь ПАРТНЁРА (колонка «они»).
 *
 * Как гостевой тест:
 * — TradePageSelectInventory(UserThem, 730, 2), #theirinventory / #trade_theirs;
 * — XHR partnerinventory и /inventory/<partner>/730/… только для partner из OWNER_TRADE_URL.
 *
 * Запуск: cd web && npm run test:puppeteer-steam-owner
 *
 * Env: OWNER_TRADE_URL, OWNER_COOKIE_HEADER, STEAM_TEST_HEADLESS=1
 * OWNER_STEAM_ID — опционально, сверка с partner из ссылки (предупреждение при расхождении).
 */
...
/** Только инвентарь партнёра по trade URL (как в гостевом тесте). Свой (UserYou) не считаем. */
...
/** economy_trade.js: TradePageSelectInventory(UserThem, appid, contextid) — инвентарь партнёра. */
...
async function waitForTradeUiPartner(page) {
...
async function main() {
  const partnerRaw = parseTradePartner(OWNER_TRADE_URL);
  if (!partnerRaw) {
    console.error("OWNER_TRADE_URL: нет параметра partner");
    process.exit(1);
  }
  const expectedPartnerSteam64 = steamId64FromPartner(partnerRaw);
  const ownerIdNorm = normalizeSteamId64ForCache(OWNER_STEAM_ID);
  if (ownerIdNorm !== expectedPartnerSteam64) {
    log("warn_owner_steam_id_vs_trade_partner", {
      expectedPartnerSteam64,
      OWNER_STEAM_ID: ownerIdNorm,
      hint: "Фильтр XHR идёт по partner из ссылки, не по OWNER_STEAM_ID",
    });
  }

  log("puppeteer_owner_start", {
    tradeUrl: OWNER_TRADE_URL,
  ...
  const pairs = parseCookieHeader(OWNER_COOKIE_HEADER);
  if (pairs.length === 0) {
    console.error("OWNER_COOKIE_HEADER: не удалось распарсить cookies.");
    process.exit(1);
  }
  ...
  try {
    ...
    const gotoUrl = OWNER_TRADE_URL.trim().startsWith("http")
      ? OWNER_TRADE_URL.trim()
      : `https://${OWNER_TRADE_URL.trim()}`;

    await page.goto(gotoUrl, { waitUntil: "networkidle2", timeout: 120_000 });
    log("page_loaded", { url: page.url() });

    await waitForTradeUiPartner(page);
    log("trade_ui_ready", { side: "partner_column" });
    ...
    while (Date.now() - waitStart < OWNER_XHR_WAIT_MS) {
